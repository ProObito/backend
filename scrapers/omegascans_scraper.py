...
class OmegaScansScraper(BaseScraper):
    def __init__(self):
        super().__init__('omegascans', 'https://omegascans.org')
    
    async def get_all_manga(self) -> List[Dict]:
        all_manga = []
        async with aiohttp.ClientSession(headers=self.headers) as session:
            html = await self.fetch_page(f"{self.base_url}/series", session)
            if not html: return []
    ...
    async def get_chapter_images(self, chapter_url: str) -> List[str]:
        async with aiohttp.ClientSession(headers=self.headers) as session:
            ...
            soup = BeautifulSoup(html, 'lxml')
            images = []
            for img in soup.select('.reading-content img, .chapter-images img'):
